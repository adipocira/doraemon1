...
void idle_entry(void* arg) {
    ...
    osCreateThread(&main_thread.thread, main_thread.thread_id = DORA_THREAD_MAIN_ID, main_entry, NULL,
                   main_thread.stackPtr = main_stack + ARRAY_COUNTU(main_stack),
                   main_thread.thread_priority = DORA_THREAD_MAIN_PRIORITY);
...
#ifndef NONMATCHING
#pragma GLOBAL_ASM("asm/nonmatchings/idle/main_entry_sub.s")
#else
void main_entry_sub(void* arg) {
    ...
    osCreateThread(&controller_thread.thread, controller_thread.thread_id = DORA_THREAD_CONTROLLER_ID, func_800016D0,
                   &D_800E6270, controller_thread.stackPtr = controller_stack - 800,
